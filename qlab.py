...
Why this rewrite?
- You hit syntax issues (e.g. `SyntaxError: unterminated string literal / f-string`) from broken string literals (e.g. stray newlines inside quotes).
- Fixed all string joins and f-strings (e.g. use `"\n".join(...)` and `f"...\n\n{var}"`).
- Keeps prior features: batch mode for sandboxes, mock quantum fallback, Streamlit UI, OpenRouter LLM chat + file uploads, and persistent chat history.
- Preserves existing tests and adds UI polish.
